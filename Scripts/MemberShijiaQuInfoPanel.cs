...
public class MemberShijiaQuInfoPanel : MonoBehaviour
{
	...
	{
		indexA = int.Parse(Mainload.MemberShijiaQuIndex_Enter.Split('|')[0]);
		indexB = int.Parse(Mainload.MemberShijiaQuIndex_Enter.Split('|')[1]);
		Mainload.PropSelected = new List<List<int>>();
		Data_A = Mainload.Member_Other_qu[indexA][indexB][2].Split('|');
		Data_B = new string[7]
		{
			Mainload.Member_Other_qu[indexA][indexB][3],
			Mathf.FloorToInt(float.Parse(Mainload.Member_Other_qu[indexA][indexB][4])).ToString(),
			Mathf.FloorToInt(float.Parse(Mainload.Member_Other_qu[indexA][indexB][5])).ToString(),
			Mathf.FloorToInt(float.Parse(Mainload.Member_Other_qu[indexA][indexB][6])).ToString(),
			Mathf.FloorToInt(float.Parse(Mainload.Member_Other_qu[indexA][indexB][7])).ToString(),
			Mainload.Member_Other_qu[indexA][indexB][8],
			Mainload.Member_Other_qu[indexA][indexB][17]
		};
		ShengYu = Mathf.FloorToInt(float.Parse(Mainload.Member_Other_qu[indexA][indexB][10])).ToString();
		StateID = Mainload.Member_Other_qu[indexA][indexB][9];
		StateTime = Mainload.Member_Other_qu[indexA][indexB][11];
		MeiLiNum = Mainload.Member_Other_qu[indexA][indexB][15];
		bodyNum = Mainload.Member_Other_qu[indexA][indexB][16];
		HuaiYunNum = Mainload.Member_Other_qu[indexA][indexB][13];
		if (Mainload.SetData[4] == 0 && float.Parse(Mainload.Member_Other_qu[indexA][indexB][12]) > 0f)
		{
			HaoGanNum = float.Parse(Mainload.Member_Other_qu[indexA][indexB][12]).ToString("f1");
		}
		else
		{
			HaoGanNum = float.Parse(Mainload.Member_Other_qu[indexA][indexB][12]).ToString("f0");
		}
		SkillLv = Mainload.Member_Other_qu[indexA][indexB][19];
		FuQiRelax = float.Parse(Mainload.Member_Other_qu[indexA][indexB][20].Split('|')[0]).ToString("f0");
		if (Data_A[4] == "0")
		{
			if (Mainload.Member_Other_qu[indexA][indexB][22] == "0")
			{
		...
		isPeiOuLive = false;
		PeiOuName = AllText.Text_UIA[1082][Mainload.SetData[4]];
		for (int i = 0; i < Mainload.Member_other[indexA].Count; i++)
		{
			if (Data_A[9] == Mainload.Member_other[indexA][i][0])
			{
				isPeiOuLive = true;
				PeiOuName = AllText.Text_UIA[1081][Mainload.SetData[4]].Replace("^", FormulaData.GetShijiaName(indexA, isNeibu: false)).Replace("@", Mainload.Member_other[indexA][i][2].Split('|')[0]).Replace("$", TipA);
				break;
			}
		}
	...
	private void OnEnableShow()
	{
		...
		obj.transform.GetComponent<PerLiHuiBig>().ShowID = 11;
			...
			.childCount; j++)
		{
			...
			int num = 0;
			if (Mainload.Member_Other_qu[indexA][indexB][24] != "null")
			{
				string[] array = Mainload.Member_Other_qu[indexA][indexB][24].Split('|');
				for (int k = 0; k < array.Length; k++)
				{
					if (int.Parse(Mainload.Member_Other_qu[indexA][indexB][3]) >= int.Parse(Mainload.AllBuffData[int.Parse(array[k].Split('@')[0])][1]))
					{
